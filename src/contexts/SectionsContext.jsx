...
export const SectionsProvider = ({ children, onDeleteSection }) => {
  ...
  const renameSection = useCallback((id, name) => {
    ...
    setSections((prev) =>
      prev.map((section) => (section.id === id ? { ...section, name: trimmed } : section))
    );
    return true;
  }, []);
  ...
  const deleteSection = useCallback(
    (id) => {
      setSections((prev) => {
        ...
        if (nextSections.length === prev.length) return prev;

        setCurrentSectionId((currentId) =>
          currentId === id ? nextSections[0]?.id ?? null : currentId
        );
  ...
  const value = useMemo(
    ...
    [sections, currentSectionId, crmName, updateCrmName, addSection, renameSection, deleteSection]
  );
